    ...
    class Fan : PhysicsEntity
    {
        ...
        private void SpawnEffect(Direction d)
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                {
                    case Direction.up:
                        effectBoxes.Add(new EffectBox(this.X, this.Y - Room.TILE_HEIGHT - (i*Room.TILE_HEIGHT), Room.TILE_WIDTH, Room.TILE_WIDTH, new Vector2(0, -25)));
                        break;
                    case Direction.right:
                        effectBoxes.Add(new EffectBox(this.X + Room.TILE_WIDTH + (i * Room.TILE_WIDTH), this.Y, Room.TILE_WIDTH, Room.TILE_WIDTH, new Vector2(25, 0)));
                        break;
                    case Direction.down:
                        effectBoxes.Add(new EffectBox(this.X, this.Y + Room.TILE_HEIGHT + (i * Room.TILE_HEIGHT), Room.TILE_WIDTH, Room.TILE_WIDTH, new Vector2(0, 25)));
                        break;
